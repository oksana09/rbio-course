...
train_me <- train_me[!rownames(train_me) == c("Position", "Chr"),]

valid_me <- as.data.frame(t(upper_cors_methylation[, validation]))
valid_me <- valid_me[!rownames(valid_me) == c("Primer"),]
...
wrapper_function <- function(train.data, train.response,
  ...
  m_RMSE_for_train <- sapply(RMSE_for_train, mean)
  
  m_RMSE_for_valid <- sapply(RMSE_for_valid, mean)
...
output <- sapply(vector_ntree, function(t) wrapper_function(train, train_response, valid, valid_response,runs.number = 100, ntree=t))
